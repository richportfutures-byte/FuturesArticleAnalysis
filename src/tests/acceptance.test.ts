...
describe('fixture-based acceptance', () => {
  it('runs at least one fixture per contract', async () => {
    ...
    for (const fixture of seedFixtures) {
      ...
      const articles = Array.isArray(fixtureAny.articles)
        ? fixtureAny.articles.map((a: any, i: number) => ({
            article_id: `${fixture.fixture_id}-${i}`,
            headline: a.headline,
            body_excerpt: a.headline,
        ...
        : [
            {
              ...
              headline: fixtureAny.article.headline,
              body_excerpt: fixtureAny.article.body_excerpt ?? fixtureAny.article.headline,
              source_type: fixtureAny.article.source_type as SourceType,
              published_at: null,
              url: null
            }
          ];
